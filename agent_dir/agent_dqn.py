...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        if args.test_dqn:
            ...
            self.policy_network.load_weights('breakouts_dqn_weights333_80.0_15190_1902194.h5') #'double_dqn_weights.h5')#
    ...
    def train(self):
        ...
        for i in range(500000):
            ...
            while (not done):
                ...
                if accumulate > history and (accumulate+1)%4 == 0:
                    ...
                    if (accumulate+1)%100 ==0:
                        self.policy_network.save_weights('breakouts_ddqn_weights111.h5')
                if accumulate > history and (accumulate+1) % 1000 == 0:
                    self.target_network.set_weights(self.policy_network.get_weights())
                if accumulate > history and explore>0.05:
                    explore = explore* 0.9999982#- 1.5*9.6e-7 #
            ...
            np.save('reward_history_ddqn111.npy', reward_history)
            if episode_reward > 30:
                self.policy_network.save_weights('breakouts_ddqn_weights111_' +
